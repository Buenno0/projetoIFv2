    ...
    <div class="response-container">
        ...
        <div>
            ...
            @elseif($statusStr === 'pendente')
                ...
                <div class="original-message-box card-full-height" style="border-color: #cbd5e1; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; min-height: 300px;">
                    ...
                    <form action="{{ route('sugestoes.iniciar_analise', $sugestao->id) }}" method="POST" class="prevent-double-submit">
                        @csrf
                        <button type="submit" class="btn btn-primary btn-lg" style="padding: 12px 24px; font-size: 1rem;">
            ...
            @else
                ...
                <div class="original-message-box card-full-height" style="border-color: #cbd5e1;">
                    ...
                    <div class="analise-info-box" style="
                        ...
                        <div>
                            <div style="font-size: 0.95rem; color: {{ $isMyAnalysis ? '#1e40af' : '#9a3412' }}; font-weight: 600;">
                                @if($isMyAnalysis)
                                    Você está analisando esta sugestão
                                @else
                                    <i class="fa-solid fa-lock" style="margin-right:5px;"></i> Em análise por {{ $analystName }}
                                @endif
                            </div>
                    ...
                    @if($isMyAnalysis)
                        ...
                        {{-- Adicionei a classe 'prevent-double-submit' --}}
                        <form action="{{ route('sugestoes.update', $sugestao->id) }}" method="POST" id="form-responder" class="prevent-double-submit">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="form-group">
                                <textarea 
                                    ...
                                    placeholder="Digite sua resposta aqui para finalizar o atendimento..."
                    ...
                    @else
                        ...
                        <div style="text-align: center; padding: 40px 20px; color: #64748b;">
                            <i class="fa-solid fa-file-shield" style="font-size: 3rem; margin-bottom: 15px; color: #cbd5e1;"></i>
                            <p>
                                <strong>Aguarde a finalização.</strong><br>
                                O usuário <strong>{{ $analystName }}</strong> está trabalhando nesta resposta no momento.<br>
            ...
            if(responseForm) {
                responseForm.addEventListener('submit', function(e) {
                    ...
                    if(valor.length < 10) {
                        ...
                        if(btn) {
                            btn.disabled = false;
                            btn.innerHTML = '<i class="fa-solid fa-paper-plane"></i> Finalizar e Enviar';
                        }
                    }
                });
            }
